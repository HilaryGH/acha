...
function AssignPartner() {
  ...
  const fetchAvailablePartners = async () => {
    ...
    try {
      setLoading(true);
      const response = await api.orders.getAvailablePartners(orderId) as { status?: string; data?: any[]; message?: string };
      if (response.status === 'success') {
  ...
  const handleAssign = async (partnerId: string) => {
    ...
    try {
      const response = await api.orders.assignToPartner(orderId, partnerId) as { status?: string; message?: string };
      if (response.status === 'success') {
        setMessage({ type: 'success', text: 'Order assigned to partner successfully!' });
        setTimeout(() => {
          navigate(`/orders/track/${orderId}`);
        }, 2000);
      } else {
        setMessage({ type: 'error', text: response.message || 'Failed to assign order' });
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Assign to Partner</h1>
          {order && (
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-2xl font-semibold text-gray-900 mb-4">Available Partners</h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-600">No available partners found.</p>
            </div>
          ) : (
                ...
                <div key={partner._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="mb-4">
                    ...
                    {partner.companyName && partner.name && (
                      <p className="text-sm text-gray-600 mb-2">{partner.companyName}</p>
                    )}
                    <div className="space-y-2 text-sm text-gray-600">
                      ...
                      {partner.primaryLocation && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>{partner.primaryLocation}</span>
                        </div>
                      )}
                      {partner.registrationType && (
                        <div className="flex items-center gap-2">
                          <span className="font-medium">Type:</span>
                          <span>{partner.registrationType}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-200 pt-4 mt-4">
                    ...
                    <button
                      ...
                      disabled={assigning || order?.status !== 'pending'}
